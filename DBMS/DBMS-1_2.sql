...
SELECT
  ...
  ROUND(COALESCE(AVG(B.Price), 0), 2) AS AvgPrice
FROM Author A
LEFT JOIN Book B ON A.AuthorID = B.AuthorID
GROUP BY A.AuthorID, A.AuthorName;
...
FROM (
  SELECT
    ...
    B.AuthorID,
    B.Price,
    ROW_NUMBER() OVER (PARTITION BY B.AuthorID ORDER BY B.Price DESC, B.PublicationYear DESC) AS rn
  FROM Book B
) t
WHERE rn = 1
ORDER BY Price DESC;
...
--3. Display Full Employee Report Write a single SQL query that joins all necessary tables to produce a report showing the following details for every employee:
--Employee Name (Ename)
